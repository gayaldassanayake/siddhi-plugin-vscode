...
export async function createConfigFile(folderPath: string): Promise<void> {
    const configPath = path.join(folderPath, 'si.yaml');
    const emptyYaml = "# AUTO-GENERATED FILE. DO NOT MODIFY. \n\n" + // TODO: take this from the LS backend if possible. Otherwise, this will be a duplicate.
                      "# This file is auto-generated by SI for managing extensions. \n" +
                      "# It should not be modified by hand.";
        try {
            await fs.promises.writeFile(configPath, emptyYaml, 'utf8');
            vscode.window.showInformationMessage('si.yaml file created successfully!');
            const document = await vscode.workspace.openTextDocument(configPath);
            await vscode.window.showTextDocument(document);
        } catch (error) {
            vscode.window.showErrorMessage(`Failed to create si.yaml: ${error}`);
        }
    }
